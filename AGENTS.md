...
- Milvus standalone boot: embedded etcd defaults (100ms heartbeat/1s election) + slow VPS disk = "etcdserver: leader changed" panic loop (exit 134). ~~Fixed via~~ Mitigated by embedEtcd heartbeat-interval 1000 / election-timeout 10000 (~~host copy ... ro-mounted — edit host file and the next restart picks it up~~ **corrected 2026-09-17: no host copy is mounted; the config is the `configs:` block in `deploy/memsearch-milvus.yaml`**). The recurring cause is an ungraceful stop mid-flush (7+ corruptions; see `docs/reviews/2026-09-07-milvus-etcd-corruption-incident-and-recovery.md`).
