...
- Seven unit tests passed for store selection, unavailable-store provenance and recovery actions, persisted agent packets, the exact MCP catalog, and active-WAL safety.
- MCP `initialize` and `tools/list` passed and returned the 14 tools named above.
